...
async def dispatch_openai_requests_x(
        messages_list,
        model: str,
        temperature: float,
        opeai_key: str,
    ...
    #     print("操作超时")
    # return await asyncio.gather(*async_responses)
    result = await asyncio.wait_for(asyncio.gather(*async_responses), timeout=int(3 * len(messages_list)))
    return result
